...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.log('🎨 Storing Basic artwork data for contract:', contractAddress);
    ...
    const artworkData = nftItems.map((item: any, index: number) => ({
      ...
      name: item.name || `Basic NFT #${index + 1}`,
      description: item.description || `A unique NFT from Basic collection`,
    ...
    if (error) {
      console.error('❌ Error storing Basic artwork:', error);
    ...
    console.log('✅ Basic artwork data stored successfully:', data?.length, 'items');

    res.status(200).json({
      success: true,
      message: 'Basic artwork data stored successfully',
      contractAddress,
    ...
    console.error('❌ Error in store-basic-artwork:', error);
    res.status(500).json({
